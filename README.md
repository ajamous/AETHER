...
| `pkg/crypto`                  | Implemented   | ECDSA + ECKA on both NIST P-256 and Brainpool P-256 r1 (SGP.22 §2.6.1 dual-curve mandate): Brainpool curve in `pkg/crypto/brainpool` (RFC 5639 params, validated against the RFC 7027 ECDH KAT); ECDSA sign/verify and a curve-agnostic ECKA agreement API (`ecka.Generate`/`DeriveBytes`, with peer-key on-curve validation) both drive it. X9.63-SHA-256 KDF, AES-128-GCM BSP. P-256 stays on constant-time `crypto/ecdh`/`crypto/ecdsa`; Brainpool's math/big arithmetic is not constant-time, so production Brainpool signing belongs in an HSM |
