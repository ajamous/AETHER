...
| `services/smdp-plus`          | Partial       | ES9+ endpoints + persistent sessions + signed `ServerSigned1` (§5.7.13) via hsm-broker DPauth + verified `EuiccSigned1` (§5.7.5) chain via certmgr trust store + DPpb-signed `SmdpSigned2` (§5.7.14) on `authenticateClient` responses; BPP wrapping (capture eUICC otPK from PrepareDownloadResponse + ECKA + KDF + AES-GCM around the SAIP UPP) is the explicit follow-up that lifts `getBoundProfilePackage` from honest 501 |
...
| Conformance harness (SGP.23)  | Implemented   | `make conformance` runs 71 cases across 10 families; coverage matrix in `tools/conformance/coverage/sgp23.md`; hardware-in-the-loop tests honestly out of scope |
