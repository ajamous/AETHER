...
| `services/smdp-plus`          | Partial       | ES9+ endpoints + persistent sessions + signed `ServerSigned1` (§5.7.13) via hsm-broker + verified `EuiccSigned1` (§5.7.5) chain via certmgr trust store + `SmdpSigned2` (§5.7.14) DER codec + DPpb sign helper; `prepareDownload` HTTP handler + BPP wrapping (ECKA + KDF + AES-GCM around the SAIP UPP) are the explicit follow-up that lifts `getBoundProfilePackage` from honest 501 |
...
| Conformance harness (SGP.23)  | Implemented   | `make conformance` runs 69 cases across 10 families; coverage matrix in `tools/conformance/coverage/sgp23.md`; hardware-in-the-loop tests honestly out of scope |
