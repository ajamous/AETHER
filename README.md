...
| `services/audit`              | Implemented   | Hash-chained ledger with verify, in-memory and Postgres-backed stores; serializable concurrent appends verified; signed timeline anchors at `/v1/anchor` (ECDSA-SHA-256 over DER-encoded `(timestamp, length, tail_hash)` SEQUENCE; opt-in via `--hsm-broker`) |
...
| Conformance harness (SGP.23)  | Implemented   | `make conformance` runs 56 cases across 9 SGP.23 + admin-auth families; coverage matrix in `tools/conformance/coverage/sgp23.md`; hardware-in-the-loop tests honestly out of scope |
...
| SAS-SM evidence templates     | Implemented   | Gap analysis, key ceremony, HSM vendor configuration, RBAC, audit retention, AWS + GCP + Azure + on-prem reference deployments, DR runbook, incident response, common audit findings, recertification checklist — all in `docs/sas-sm/` (free, no paid tier) |
