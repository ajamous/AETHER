...
| `pkg/saip`                    | Partial       | Minimum-viable SGP.22 §B SAIP codec: ProfileHeader + PEEnd, DER round-trip, AppendRaw for spare ProfileElements. Richer ProfileElements (PE-USIM, PE-PinCodes, PE-FileSystem, PE-AKAParameter, …) land as the catalogue grows |
...
| `services/smdp-plus`          | Partial       | ES9+ endpoints + persistent sessions + signed `ServerSigned1` (§5.7.13) + verified `EuiccSigned1` (§5.7.5) + DPpb-signed `SmdpSigned2` (§5.7.14) + BPP session-key derivation (§H.3 ECKA → SCP03t SENC/SMAC/MCV) + AES-128-GCM segment seal/open with MAC chaining + outer `BoundProfilePackage` (§5.7.6) SEQUENCE assembly with `InitialiseSecureChannelRequest` (§5.7.7) — every BPP layer codec ships in `internal/bpp` and the `getBoundProfilePackage` handler now returns a real DER-encoded BPP when DPpb is configured (lab mode without DPpb still returns honest 501). Spec-precise per-segment AAD layout + signature-verified PrepareDownloadResponse parsing remain hardware-bench follow-ups |
...
| `services/profile-builder`    | Partial       | YAML template loader + UPP envelope + real DER-encoded SAIP via `pkg/saip` (header + PEEnd today; richer ProfileElements land as `pkg/saip` grows) |
...
| Conformance harness (SGP.23)  | Implemented   | `make conformance` runs 88 cases across 10 families; coverage matrix in `tools/conformance/coverage/sgp23.md`; hardware-in-the-loop tests honestly out of scope |
